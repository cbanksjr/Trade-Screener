...
Schwab adds an Options Positioning layer after the base scan and optional FMP context pass. It derives call/put activity, estimated option premium, volume relative to open interest, unsigned gamma concentration by strike, max pain, and overnight call open-interest change from scan-time option-chain snapshots. These are positioning proxies, not proof of institutional identity, trade direction, opening/closing intent, or multi-leg execution. A genuine open-interest build requires a comparable snapshot from the immediately preceding trading session; same-session volume cannot confirm positioning by itself. Positioning enrichment is limited to qualifying or tracked setups and capped at 50 requests per scan by default; override that guard with `SCHWAB_POSITIONING_MAX_CALLS_PER_SCAN`.
