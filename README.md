...
QuantData adds the live Institutional Positioning layer after the base scan and optional FMP context pass. It uses prior-session options net drift/order flow, exposure by strike, and dark-pool levels to mark each candidate as `confirmed`, `neutral`, `capped`, or `vetoed`. Bullish positioning can confirm a clean setup, while bearish flow, hostile call-wall exposure, or distribution-like dark-pool data can mark an A/B setup as Avoid without changing its setup grade. Configure it with `QUANTDATA_API_KEY`, `QUANTDATA_BASE_URL=https://api.quantdata.us`, `QUANTDATA_ENABLED=true`, `QUANTDATA_MAX_CALLS_PER_SCAN=300`, and `QUANTDATA_CACHE_TTL_MINUTES=15`.
