...
- Optional QuantData universe-level Net Flow / Gainers-Losers ranking reorders (does not filter) the per-scan symbol list once per scan so QuantData's per-symbol call budget is spent on names with real same-day institutional flow first; it is a scan-ordering signal, not a scoring factor
